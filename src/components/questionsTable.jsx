...
class QuestionsTable extends Component {
  ...
  async doRefresh () {
    ...
    try {
      let result = await fetchAPI({
        ...
        path: 'questions',
        filterParams: this.state.topicFilter
  ...
  toggleComponentBusy () {
    this.setState({
      componentBusy: this.state.componentBusy ? null : 'componentBusy'
    })
  }
  ...
  toggleTopicFilterBusy () {
    this.setState((prevState) => ({
      topicFilterBusy: !prevState.topicFilterBusy
    }))
  }
  ...
  toggleAnswersFormOpen () {
    this.setState((prevState) => ({
      answersFormOpen: !prevState.answersFormOpen
    }))
  }
  ...
  render () {
    return (
      <div
        tabIndex='0'
        onKeyDown={
          this.state.formOpen || this.state.answersFormOpen
            ? null
            : this.handleKeyDown
        }
      >
        ...
        <section className={this.state.componentBusy}>
          ...
          &nbsp;
          <a href='#cleanFilter' onClick={this.handleCleanFilter}>
            🆑 Limpiar filtro
          </a>
          <table>
            <thead>
          ...
          <a
            ...
            role='button'
            className='secondary'
            disabled={!this.state.questionSelected}
        ...
        <button className='primary' onClick={this.handleNewButton}>
          Nueva Pregunta
        </button>
        <section id='answersSection' className='grid'>
          <div>
            <h5 aria-busy={this.state.moreInfoBusy}>Tema</h5>
            {this.state.questionSelected
              ? (
